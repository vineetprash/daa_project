...
from algorithms import valid, solve, find_empty, generate_board
...
class Board:
    ...
    def redraw(self, keys, wrong, time):
        ...
        Returns:
            None
        """
        self.window.fill((112, 102, 119))  # fill the window with white
        ...
        if wrong > 0:
            ...
            text = font.render(str(wrong), True, (255, 255, 200))
            self.window.blit(text, (32, 542))

        # display the current time elapsed as a number
        font = pygame.font.SysFont("Bahnschrift", 40)
        text = font.render(str(time), True, (255, 255, 200))
    ...
    def visualSolve(self, wrong, time):
        ...
        for nums in range(9):
            if valid(self.board, (empty[0], empty[1]), nums + 1):
                ...
                pygame.time.delay(self.DELAY_TIME)  # delay to slow down the solving animation
                self.redraw(
                    {}, wrong, time
...
def main():
    ...
    # Splash screen
    font = pygame.font.SysFont("Bahnschrift", 40)
    text = font.render("Generating", True, (255, 255, 200))
    screen.blit(text, (175, 245))
    font = pygame.font.SysFont("Bahnschrift", 40)
    text = font.render("Random Grid", True, (255, 255, 200))
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_SPACE:
                    ...
                    passedTime = time.strftime("%H:%M:%S", time.gmtime(time.time() - solveStartTime))
                    for i in range(9):
                        for j in range(9):
                    ...
                    solved = True
                    board.redraw({}, wrong, "Solved in: " + passedTime)
                    break
